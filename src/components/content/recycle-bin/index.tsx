...
const RecycleItem: React.FC<RecycleItemProps> = ({ text, image, isSelected, onClick }) => {
  return (
    <div onClick={onClick} className='flex flex-col items-center w-16 md:w-20 m-1 cursor-default select-none group'>
      <div className='relative p-1 icon-selected-wrapper'>
        ...
        <img
          ...
          className={`w-10 h-10 md:w-12 md:h-12 object-contain [image-rendering:pixelated] ${
            isSelected ? 'icon-selected-image' : ''
          }`}
        />
      </div>
      <span
        className={`mt-1 px-1 text-[10px] md:text-[11px] text-center break-all leading-tight border border-transparent ${
  ...
  return (
    <div className='flex flex-col h-full bg-white font-sans'>
      <div className='flex flex-1 overflow-hidden'>
        ...
        <div
          className='flex-1 p-4 flex flex-wrap content-start gap-2 md:gap-4 overflow-y-auto bg-white select-none'
      ...
      <div className='flex justify-between items-center px-2 py-1 bg-[#C0C0C0] border-t-2 border-white shadow-[0_-1px_0_0_#808080] text-[10px] md:text-[11px] text-black'>
        <div className='flex gap-2 h-4 items-center'>
          <div className='px-1 md:px-2 border-r border-gray-400 h-full flex items-center'>
            {trashItems.length} object(s)
          </div>
          <div className='px-1 md:px-2 h-full flex items-center'>{selectedId !== null ? '1 selected' : ''}</div>
        </div>
        <div className='px-2 md:px-4 border-l border-gray-400 h-full flex items-center font-bold'>94.2 MB</div>
      </div>
    </div>
  );
};
